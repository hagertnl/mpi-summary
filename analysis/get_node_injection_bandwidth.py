        ...
        for line in in_f:
            ...
            for recv_from_rank in trace_obj.recv_from.keys():
                if recv_from_rank == 'all':
                    ...
                    recv_data_by_time[time_bin] += trace_obj.recv_from[recv_from_rank]
            # Add sent bytes:
            for sent_to_rank in trace_obj.sent_to.keys():
                if sent_to_rank == 'all':
                    ...
                    time_bin = int((trace_obj.start - args.starttime) / args.timestep)
                    send_data_by_time[time_bin] += trace_obj.sent_to[sent_to_rank]

for i in range(0, len(index)):
    send_data_by_time[i] = send_data_by_time[i] / args.timestep / pow(10,9)
    recv_data_by_time[i] = recv_data_by_time[i] / args.timestep / pow(10,9)

# Plot #########################################################################
plt.title(f'Bytes sent & received by node {args.nodenumber}', fontsize = 16)
# axis labels
plt.xlabel('Timestamp', fontsize = 12)
plt.ylabel('GBytes/sec', fontsize = 12)
...
plt.savefig('node_injection_bandwidth.png', dpi=300)
